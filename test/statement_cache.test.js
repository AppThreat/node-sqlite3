...
describe('statement cache', function() {
    ...
    beforeEach(function(done) {
        db = new sqlite3.Database(':memory:', function(err) {
            assert.ifError(err);
            db.exec('CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER, b TEXT, c BLOB)', done);
        });
    });
    ...
    it('is off by default and does not change behavior', function(done) {
    ...
    it('reuses statements with per-call bind values', function(done) {
        db.cacheStatements();
        const vals = [];
        ...
        vals.forEach(function(v, i) {
            db.run('INSERT INTO t (a, b) VALUES (?, ?)', i, v, function(err) {
                assert.ifError(err);
                if (++n === vals.length) {
                    db.all('SELECT a, b FROM t ORDER BY id', function(err, rows) {
                        assert.ifError(err);
    ...
    it('re-runs statements bound with no parameters', function(done) {
        db.cacheStatements();
        for (let i = 0; i < 5; i++) {
    ...
    it('supports get/all/each/map through the cache', function(done) {
        db.cacheStatements();
        ...
        function check() {
            db.get('SELECT a, b, c FROM t WHERE a = ?', 2, function(err, row) {
                ...
                db.all('SELECT a FROM t ORDER BY a', function(err, rows) {
                    assert.ifError(err);
                    assert.deepStrictEqual(rows.map(r => r.a), [0, 1, 2, 3]);
                    let seen = 0;
                    db.each('SELECT a FROM t ORDER BY a', function(err, row) {
                        assert.ifError(err);
                        assert.strictEqual(row.a, seen++);
                    }, function(err, count) {
                        assert.ifError(err);
                        assert.strictEqual(count, 4);
                        db.map('SELECT a, b FROM t ORDER BY a', function(err, result) {
                            assert.ifError(err);
    ...
    it('keeps named-parameter and object binds per call', function(done) {
        db.cacheStatements();
        db.run('INSERT INTO t (a, b) VALUES ($a, $b)', {$a: 1, $b: 'one'}, function(err) {
            assert.ifError(err);
            db.run('INSERT INTO t (a, b) VALUES ($a, $b)', {$a: 2, $b: 'two'}, function(err) {
                assert.ifError(err);
                db.all('SELECT a, b FROM t ORDER BY a', function(err, rows) {
                    assert.ifError(err);
    ...
    it('keeps FIFO ordering on the same cached statement in one tick', function(done) {
        db.cacheStatements();
        const N = 100;
        for (let i = 0; i < N; i++) {
    ...
    it('defers to the uncached path under serialize() and keeps FIFO order', function(done) {
        db.cacheStatements();
        const order = [];
        db.serialize(function() {
            for (let i = 0; i < 30; i++) {
                db.run('INSERT INTO t (a) VALUES (?)', i, function() { order.push(i); });
            }
        });
        db.wait(function() {
            // serialize() must complete callbacks in issue order
            for (let i = 0; i < 30; i++) {
                assert.strictEqual(order[i], i, 'serialize order broken at ' + i);
            }
    ...
    it('survives LRU eviction pressure with a tiny cache', function(done) {
    ...
    it('reports prepare errors and recovers', function(done) {
        db.cacheStatements();
    ...
    it('reports runtime errors per call without poisoning the cache', function(done) {
        db.cacheStatements();
        db.run('CREATE TABLE IF NOT EXISTS u (x INTEGER UNIQUE)', function(err) {
            assert.ifError(err);
            db.run('INSERT INTO u (x) VALUES (?)', 1, function(err) {
                ...
                db.run('INSERT INTO u (x) VALUES (?)', 1, function(err) {
                    ...
                    db.run('INSERT INTO u (x) VALUES (?)', 2, function(err) {
                        assert.ifError(err);
                        done();
                    });
                });
    ...
    it('coexists with user-held prepared statements', function(done) {
        db.cacheStatements();
    ...
    describe('close interaction', function() {
        ...
        beforeEach(function(done) {
            ...
            db.close(function() {
                db = new sqlite3.Database(FILE, function(err) {
                    assert.ifError(err);
                    db.exec('CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER)', done);
                });
            });
        });

        it('flushes cached statements on close (no SQLITE_BUSY)', function(done) {
            db.cacheStatements();
            for (let i = 0; i < 20; i++) {
                db.run('INSERT INTO t (a) VALUES (?)', i);
            }
            db.close(function(err) {
                assert.ifError(err);
                db = new sqlite3.Database(FILE, sqlite3.OPEN_READONLY, function(err2) {
                    assert.ifError(err2);
                    db.get('SELECT COUNT(*) AS n FROM t', function(e, row) {
                        assert.ifError(e);
        ...
        it('delivers queued cached callbacks before close completes', function(done) {
            db.cacheStatements();
            let callbacks = 0;
            for (let i = 0; i < 50; i++) {
                db.run('INSERT INTO t (a) VALUES (?)', i, function() { callbacks++; });
            }
        ...
        it('closes cleanly with cache enabled but empty', function(done) {
            db.cacheStatements();
            db.close(function(err) {
                assert.ifError(err);
                // keep afterEach happy: already closed
                db = new sqlite3.Database(':memory:', function() { done(); });
            });
        });
    });
});
...
describe('statement cache ordering vs exclusive operations', function() {
    it('does not let a cached statement overtake exec()', function(done) {
        const order = [];
        ...
        db.run('CREATE TABLE t (i)', function() {
            ...
            db.run('INSERT INTO t VALUES (1)', function() {
                db.exec('INSERT INTO t VALUES (2); INSERT INTO t VALUES (3);',
                    function(err) {
                        assert.ifError(err);
                        order.push('exec');
                    });
    ...
    it('rejects a cached statement issued after close()', function(done) {
        const db = new sqlite3.Database(':memory:');
        db.cacheStatements();
        db.run('CREATE TABLE t (i)', function() {
            db.run('INSERT INTO t VALUES (1)', function() {
    ...
    it('rejects a cached statement issued after close() from a callback',
       function(done) {
        const db = new sqlite3.Database(':memory:');
        db.cacheStatements();
        db.run('CREATE TABLE t (i)', function() {
            db.run('INSERT INTO t VALUES (1)', function() {
                ...
                db.close(function(err) { assert.ifError(err); });
                db.run('INSERT INTO t VALUES (1)', function(err) {
                    assert.ok(err, 'run after close() must not succeed');
                    done();
                });
