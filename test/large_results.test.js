...
describe('large result delivery', function() {
    ...
    it('delivers a large result set correctly', function(done) {
        this.timeout(30000);
        const N = 100000;
    ...
    it('fires callbacks in queue order regardless of result size', function(done) {
        this.timeout(30000);
        const order = [];
        db.serialize(function() {
            db.all('SELECT id, txt FROM foo LIMIT 100000', function(err, rows) {
                assert.ifError(err);
    ...
    it('does not drop the callback when the db is closed in the same tick',
       function(done) {
        this.timeout(30000);
        ...
        let called = false;
        db2.all('SELECT id, txt FROM foo LIMIT 100000', function(err, rows) {
            assert.ifError(err);
            assert.strictEqual(rows.length, 100000);
            called = true;
        });
        db2.close(function(err) {
            assert.ifError(err);
            assert.ok(called, 'all() callback must fire before close completes');
            done();
        });
    });
});
