...
describe('backup', function () {
    ...
    afterEach(function (_t, done) {
        // Tests that close `db` themselves set it to null; without the early
        // return the original mocha hook fell through to db.close(done) on a
        // null db and called done() twice. node:test, unlike mocha, fails the
        // suite on that, so the intended early exit is now explicit.
        if (!db) {
            return done();
        }
        db.close(done);
    ...
    it('output db created once step is called', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db', function (err) {
            if (err) throw err;
    ...
    it('copies source fully with step(-1)', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db');
        backup.step(-1, function (err) {
            if (err) throw err;
            fileExists('test/tmp/backup.db');
    ...
    it('backup db not created if finished immediately', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db');
        backup.finish(function (err) {
            if (err) throw err;
    ...
    it('error closing db if backup not finished', function (_t, done) {
        const _backup = db.backup('test/tmp/backup.db');
    ...
    it('using the backup after finished is an error', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db');
        backup.finish(function (err) {
            if (err) throw err;
            backup.step(1, function (err) {
    ...
    it('remaining/pageCount are available after call to step', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db');
    ...
    it('backup works if database is modified half-way through', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db');
        backup.step(-1, function (err) {
            if (err) throw err;
            backup.finish(function (err) {
                ...
                const db2 = new sqlite3.Database(
                    ...
                    function (err) {
                        if (err) throw err;
                        const backup2 = db2.backup('test/tmp/backup2.db');
    ...
    it('cannot backup to a locked db', function (_t, done) {
        const db2 = new sqlite3.Database('test/tmp/backup.db', function (_err) {
    ...
    it('fuss-free incremental backups work', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db');
        let timer;
    ...
    it('setting retryErrors to empty disables automatic finishing', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db');
        backup.retryErrors = [];
        backup.step(-1, function (err) {
            if (err) throw err;
    ...
    it('setting retryErrors enables automatic finishing', function (_t, done) {
        const backup = db.backup('test/tmp/backup.db');
    ...
    it('default retryErrors will retry on a locked/busy db', function (_t, done) {
        const db2 = new sqlite3.Database('test/tmp/backup.db', function (_err) {
            db2.exec('PRAGMA locking_mode = EXCLUSIVE');
            db2.exec('BEGIN EXCLUSIVE', function (err) {
                if (err) throw err;
