...
//    same shape: it starts the close (the connection is unusable at once)
//    and the handle is released on the queue — `await db[Symbol.
//    asyncDispose]()`, or `await db.native.close()`, when the close must
//    have completed (deleting the file, reopening it on Windows).
...
class StatementSync {
     ...
     */
    constructor(stmt, options, reprepare) {
        this.#stmt = stmt;
...
class DatabaseSync {
    ...
    prepare(sql, options = {}) {
        ...
        return new StatementSync(stmt, options, prepare);
    }
    ...
    /**
     * Closes the connection. Divergence: the close is queued (this
     * package's close is asynchronous), so the connection refuses further
     * work at once but the file handle is released a turn later — use
     * `await using` / `Symbol.asyncDispose` when a caller must know the
     * file is free, e.g. before deleting or reopening it on Windows.
     *
     * @returns {void}
     */
    close() {
