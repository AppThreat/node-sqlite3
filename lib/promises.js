 ...
 * `untrusted`).
 *
 * @typedef {(filename: string, modeOrOptions?: number | import('./sqlite3.js').OpenOptions) => Promise<import('./native.js').Database>} OpenFunction
 * @since 9.0.0
 */
...
/** @type {WeakMap<object, import('./sqlite3-binding.js').Database>} */
...
// Cost (measured on this branch, D08, closing the D05→D07 carry-over):
// the wrapper adds ~3 µs over a raw `await db.exec('BEGIN');
// await db.exec('COMMIT')` pair (17.2 vs 14.3 µs for an empty body) —
// the AsyncLocalStorage enter/exit, the per-call Map copy and option
// validation. The remaining ~14 µs is the exclusive-queue round trips a
// hand-rolled pair pays too. AsyncLocalStorage is not optional here:
// per-connection depth must survive awaits for correct nesting, and it
// is Node's only mechanism for that; a synchronous depth counter is
// exactly what D05's blocking finding replaced. Accepted by design.
 ...
 * @param {(tx: import('./sqlite3-binding.js').Database) => unknown} fn the body.
 ...
 * @param {typeof import('./sqlite3-binding.js').Session} Session the class.
 * @param {typeof import('./sqlite3-binding.js').Blob} Blob the class.
 ...
 * @param {Record<string, (...callArgs: any[]) => any>} sessionCores
 * @param {Record<string, (...callArgs: any[]) => any>} blobCores
 * @returns {void}
 * @private
 */
function install(
    ...
    // Deliverable 07: value-returning scheduled work. Dual-mode with the
    // callback as the settled value.
    Database.prototype.checkpoint = dualMode(dbCores.checkpoint);
    Database.prototype.tableInfo = dualMode(dbCores.tableInfo);
    Database.prototype.dbConfig = dualMode(dbCores.dbConfig);
    ...
    // Deliverable 08: sessions, changesets, serialization and blob I/O.
    // applyChangeset resolves the (usually absent) rebase buffer: a plain
    // apply calls back with no second argument, so the default pick
    // resolves undefined; { rebase: true } resolves the harvested bytes.
    Database.prototype.serializeToBytes = dualMode(dbCores.serializeToBytes);

    Session.prototype.changeset = dualMode(sessionCores.changeset);
    Session.prototype.patchset = dualMode(sessionCores.patchset);
    Session.prototype.diff = dualMode(sessionCores.diff, { void: true });
    Session.prototype.close = dualMode(sessionCores.close, { void: true });

    Blob.prototype.read = dualMode(blobCores.read, {
        pick: (_self, transferred) => transferred,
    });
    Blob.prototype.write = dualMode(blobCores.write, {
        pick: (_self, transferred) => transferred,
    });
    Blob.prototype.reopen = dualMode(blobCores.reopen, { void: true });
    Blob.prototype.close = dualMode(blobCores.close, { void: true });

    Database.prototype.applyChangeset = dualMode(dbCores.applyChangeset);
     ...
     * promise-native form. `new Database(...)` is unchanged. The second
     * argument is either open flags or a v9 options object (with `mode`
     * and `untrusted`).
     *
     * @param {string} filename the database file (or `:memory:` / `''`).
     * @param {number | import('./sqlite3.js').OpenOptions} [modeOrOptions]
     *   open flags, e.g. `sqlite3.OPEN_READWRITE`, or
     *   `{ mode, untrusted }`.
     ...
     * @example
     * const hostile = await sqlite3.open('downloaded.db', {
     *     mode: sqlite3.OPEN_READONLY,
     *     untrusted: true,
     * });
     */
    sqlite3.open = function open(filename, modeOrOptions) {
        return new Promise((resolve, reject) => {
            /** @type {import('./sqlite3-binding.js').Database} */
            let db;
            try {
                ...
                db =
                    modeOrOptions === undefined || modeOrOptions === null
                        ? new OpenCtor(filename, onOpen)
                        : new OpenCtor(filename, modeOrOptions, onOpen);
            } catch (err) {
                reject(err);
            }
        });
     ...
     * statement cache).
     *
     * @param {(tx: import('./sqlite3-binding.js').Database) => unknown} fn the transaction body, receives `(tx)`.
     * @param {object} [options]
    ...
    Database.prototype.transaction = function (fn, options = {}) {
        ...
        return runTransaction(
            this,
            /** @type {(tx: import('./sqlite3-binding.js').Database) => unknown} */ (
                fn
            ),
            {
                mode,
    ...
    /**
     * The reusable transaction form (Phase 1): returns a wrapped function
     * instead of running the body once — better-sqlite3/bun/Deno's shape,
     * but async-aware (the body may await, and nesting still becomes a
     * savepoint through the same AsyncLocalStorage tracking the inline
     * form uses).
     *
     * The wrapper carries `.deferred()`, `.immediate()` and
     * `.exclusive()` variants that override the begin mode per call; the
     * default is whichever mode was passed here (`'deferred'` unless
     * said otherwise — pass `'immediate'` for write-heavy bodies: it
     * takes the write lock up front and avoids the deferred-write
     * upgrade failure under concurrency).
     *
     * @param {(tx: import('./sqlite3-binding.js').Database, ...args: unknown[]) => unknown} fn
     *   the transaction body; receives `(tx, ...args)`.
     * @param {object} [options]
     * @param {'deferred' | 'immediate' | 'exclusive'} [options.mode='deferred']
     ...
     * @since 9.1.0
     * @example
     * const move = db.createTransaction((tx, from, to, amount) =>
     *     tx.run('UPDATE accounts SET bal = bal - ? WHERE id = ?', amount, from)
     *         .then(() => tx.run('UPDATE accounts SET bal = bal + ? WHERE id = ?', amount, to)),
     * );
     * await move(1, 2, 50);
     * await move.immediate(2, 1, 50);
     */
    Database.prototype.createTransaction = function (fn, options = {}) {
        if (typeof fn !== 'function') {
            throw new TypeError('createTransaction() requires a function body');
        }
        const mode = options?.mode ?? 'deferred';
        if (!TRANSACTION_MODES.has(mode)) {
            throw new TypeError(
                "createTransaction() mode must be 'deferred', 'immediate' or 'exclusive'",
            );
        }
        const base = {
            savepoint: options?.savepoint === true,
         ...
         */
        const invoke = (beginMode, args) =>
            runTransaction(this, (tx) => fn(tx, ...args), {
                ...base,
                mode: beginMode,
            });
        /**
         ...
         * @returns {Promise<unknown>}
         */
        const wrapper = (...args) => invoke(mode, args.slice());
        /**
         * @param {...unknown} args
         * @returns {Promise<unknown>}
         */
        wrapper.deferred = (...args) => invoke('deferred', args.slice());
        /**
         * @param {...unknown} args
         * @returns {Promise<unknown>}
         */
        wrapper.immediate = (...args) => invoke('immediate', args.slice());
        /**
         * @param {...unknown} args
         * @returns {Promise<unknown>}
         */
        wrapper.exclusive = (...args) => invoke('exclusive', args.slice());
        return wrapper;
    };
    ...
    // --- Sessions and blobs (Deliverable 08): disposal and streams ----

    /**
     * `using` support for a session: initiates an asynchronous close
     * synchronously. The callback keeps it in callback mode so no promise
     * — and no possible unhandled rejection — is created.
     *
     * @since 9.0.0
     */
    Session.prototype[Symbol.dispose] = function () {
        this.close(function () {
            /* best effort */
        });
    };
    ...
    /**
     * `await using` support: closes the session.
     *
     * @returns {Promise<void>} resolves once closed.
     * @since 9.0.0
     */
    ...
    /**
     * `using` support for a blob handle: initiates an asynchronous close
     * synchronously.
     *
     * @since 9.0.0
     */
    Blob.prototype[Symbol.dispose] = function () {
        this.close(function () {
            /* best effort */
        });
    };
    ...
    /**
     * `await using` support: closes the blob handle.
     *
     * @returns {Promise<void>} resolves once closed.
     * @since 9.0.0
     */
    Blob.prototype[Symbol.asyncDispose] = function () {
        return new Promise((resolve, reject) => {
            /**
             * @param {Error | null} err
             */
    ...
    /**
     * Streams the blob's bytes as a binary `Readable`, reading in chunks
     * so memory stays flat regardless of the blob's size. The size is
     * captured once at creation; a row write mid-stream surfaces as the
     * stream's error (`SQLITE_ABORT`).
     *
     * @this {import('./sqlite3-binding.js').Blob}
     * @param {{ highWaterMark?: number }} [options] the chunk size
     *   (default 64 KiB).
     * @returns {import('node:stream').Readable} a binary stream of the blob.
     * @since 9.0.0
     * @example
     * await pipeline(blob.createReadStream(), fs.createWriteStream(path));
     */
    Blob.prototype.createReadStream = function (options) {
        const blob = this;
        const chunkSize = options?.highWaterMark ?? 64 * 1024;
        const size = /** @type {number} */ (blob.size);
        let offset = 0;
        return new Readable({
            ...
            read() {
                ...
                blob.read(chunk, offset, (err, transferred) => {
                    if (err) {
                        this.destroy(err);
                        return;
                    }
    ...
    /**
     * Returns a binary `Writable` that appends into the blob at a moving
     * offset, chunk by chunk. SQLite's incremental-blob API cannot grow a
     * blob: size the column first (e.g. `UPDATE ... SET data =
     * zeroblob(n)`) and then stream into it.
     *
     * @this {import('./sqlite3-binding.js').Blob}
     * @param {{ highWaterMark?: number }} [options] the chunk size
     *   (default 64 KiB).
     * @returns {import('node:stream').Writable} a binary stream into the blob.
     * @since 9.0.0
     * @example
     * await pipeline(fs.createReadStream(path), blob.createWriteStream());
     */
    Blob.prototype.createWriteStream = function (options) {
        const blob = this;
        const chunkSize = options?.highWaterMark ?? 64 * 1024;
        let offset = 0;
        return new Writable({
            ...
            write(chunk, _encoding, callback) {
                blob.write(chunk, offset, (err, transferred) => {
                    offset += transferred;
                    callback(err);
                });
            },
...
export function installPromiseApi(sqlite3) {
    ...
    dbCores = {
        ...
        checkpoint: Database.prototype.checkpoint,
        tableInfo: Database.prototype.tableInfo,
        dbConfig: Database.prototype.dbConfig,
        applyChangeset: Database.prototype.applyChangeset,
        serializeToBytes: Database.prototype.serializeToBytes,
    };
    stmtCores = {
    ...
    sessionCores = {
        changeset: Session.prototype.changeset,
        patchset: Session.prototype.patchset,
        diff: Session.prototype.diff,
        close: Session.prototype.close,
    };
    blobCores = {
        read: Blob.prototype.read,
        write: Blob.prototype.write,
        reopen: Blob.prototype.reopen,
        close: Blob.prototype.close,
    };
    install(
        sqlite3,
        Database,
        ...
        /** @type {Record<string, (...callArgs: any[]) => any>} */ (blobCores),
    );
}
...
export function retracePromiseApi(extendTrace) {
    ...
    /** @type {Record<string, (...callArgs: any[]) => any>} */
    const session = /** @type {Record<string, (...callArgs: any[]) => any>} */ (
        /** @type {unknown} */ (sessionCores)
    );
    /** @type {Record<string, (...callArgs: any[]) => any>} */
    const blob = /** @type {Record<string, (...callArgs: any[]) => any>} */ (
        /** @type {unknown} */ (blobCores)
    );
    ...
    for (const name of Object.keys(session)) {
        extendTrace(session, name);
    }
    for (const name of Object.keys(blob)) {
        extendTrace(blob, name);
    }
    install(
        ...
        installedNow.Backup,
        installedNow.sqlite3.Session,
        installedNow.sqlite3.Blob,
        /** @type {Record<string, (...callArgs: any[]) => any>} */ (dbCores),
        /** @type {Record<string, (...callArgs: any[]) => any>} */ (stmtCores),
        /** @type {Record<string, (...callArgs: any[]) => any>} */ (
            backupCores
