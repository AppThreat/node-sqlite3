...
class SqlitePool {
    ...
    // The filename is a URI asserting read-only access (`mode=ro`,
    // `immutable=1`). Every connection — the writer included — then opens
    // without OPEN_CREATE, so a path that does not exist fails loudly
    // instead of being created: `mode=ro` was already refused by SQLite
    // itself (a URI mode may only narrow the flags), but `immutable=1`
    // is not a mode, so the writer's OPEN_CREATE used to fabricate a
    // 0-byte database and every read then returned nothing, silently.
    /** @type {boolean} */
    #readOnlyFile;
    ...
    constructor(filename, options) {
        ...
        this.#readOnlyFile = parsePoolUri(filename).readonly;
    }
    ...
    #spawn(readOnly, spawned) {
        ...
        slot.worker.postMessage({
            ...
            readOnly,
            readOnlyFile: this.#readOnlyFile,
...
/**
 * Reads the parts of a SQLite URI filename the pool has to act on.
 *
 * Only the query parameters matter here (the path is the worker's and
 * SQLite's business), and only three of them: `mode`, `immutable` and
 * `cache`. A non-URI filename reports `uri: false` and nothing else.
 *
 * @param {string} filename the filename as given to pool().
 * @returns {{ uri: boolean, readonly: boolean, memory: boolean,
 *     sharedCache: boolean }} what the URI asks for.
 * @private
 */
function parsePoolUri(filename) {
    ...
    const params = new URLSearchParams(q === -1 ? '' : filename.slice(q + 1));
    let path = filename.slice('file:'.length, q === -1 ? undefined : q);
    try {
        path = decodeURIComponent(path);
    } catch {
        // Left as written: a malformed escape is SQLite's error to report,
        // and the only thing read from the path here is ':memory:'.
    }
    const mode = params.get('mode');
    return {
        uri: true,
        readonly: mode === 'ro' || params.get('immutable') === '1',
        memory: mode === 'memory' || path.endsWith(':memory:'),
        sharedCache: params.get('cache') === 'shared',
    };
}
...
async function pool(filename, options) {
    ...
    const uri = parsePoolUri(filename);
    const inMemory =
        filename === ':memory:' ||
        filename === '' ||
        (uri.memory && !uri.sharedCache);
    if (opts.readers > 0 && inMemory) {
        const what =
            filename === ''
                ? "''"
                : uri.memory
                  ? `the in-memory URI '${filename}'`
                  : "':memory:'";
        throw new TypeError(
            `pool() cannot open ${what} ` +
                ...
                (uri.memory ? ' (or add cache=shared to the URI)' : ''),
        );
    }
    if (uri.readonly && opts.walMode) {
        // Enabling WAL writes to the file; on a read-only URI the writer
        // worker's `PRAGMA journal_mode = WAL` cannot succeed, and the
        // pool would fail to open at all. WAL is on by default, so a
        // default is quietly dropped and only an explicit request is an
        // error — silently ignoring what the caller asked for would be
        // the worse half of the trade.
        if (/** @type {any} */ (options)?.walMode === true) {
            throw new TypeError(
                `pool() cannot enable WAL on the read-only URI '${filename}': ` +
                    'switching journal mode is a write. Pass walMode: false, ' +
                    'or open the file read-write',
            );
        }
        opts.walMode = false;
    }
    if (uri.memory || filename === ':memory:') {
        // Same treatment for an in-memory database, which has no journal
        // to switch: `PRAGMA journal_mode = WAL` reports 'memory' and the
        // writer's check turned that into "could not enable WAL mode",
        // failing the two documented in-memory pool forms — `readers: 0`
        // and the `cache=shared` URI the readers>0 error message
        // recommends — under nothing but the default options.
        if (opts.walMode && /** @type {any} */ (options)?.walMode === true) {
            throw new TypeError(
                `pool() cannot enable WAL on the in-memory database '${filename}': ` +
                    "an in-memory database has no journal (PRAGMA journal_mode reports 'memory'). " +
                    'Pass walMode: false, or use a file',
            );
        }
        opts.walMode = false;
    }
